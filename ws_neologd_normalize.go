/*
 MySQL UDF ws neologd normalize
*/
package main
...
//export ws_neologd_normalize_init
func ws_neologd_normalize_init(initid *C.UDF_INIT, args *C.UDF_ARGS, message *C.char) C.my_bool {
...
//export ws_neologd_normalize
func ws_neologd_normalize(initid *C.UDF_INIT, args *C.UDF_ARGS, result *C.char, length *C.ulong, isNull *C.char, error *C.char) *C.char {
	...
	// -----
	// ここが処理本体
	rslt := _func.NeoLogdNormalize(argsString[0])
	// -----
...
//export ws_neologd_normalize_deinit
func ws_neologd_normalize_deinit(initid *C.UDF_INIT) {
	C.free(unsafe.Pointer(initid.ptr))
}
